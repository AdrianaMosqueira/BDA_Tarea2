...
author: "A. Mosqueira, A. Palacios, E. Quispe, "
date: "05/06/2022"
output:
  rmdformats::material:
  code_folding: show
...
Además, para ambos casos se considero la vivienda particular como la unidad secundaria de muestreo (USM). 
...
Para la declaración del diseño muestral, dado que la muestra es estratificada y bietápica, se identificaron algunas observaciones que no pertenecen a ningun a estrato, específicamente en las observaciones correspondientes al área rural. Estas observaciones tuvieron que ser eliminadas para poder declarar el diseño muestral tomando en cuenta la variable estrato identificada por la encuesta, denominada "ESTRATO".

```{r}
ENAPRES <- ENAPRES[!is.na(ENAPRES$ESTRATO), ]
ENAPRES <- ENAPRES[!is.na(ENAPRES$P208_A), ]
```

Prodemos a identificar la cantidad de clusters que existen para la muestra
```{r}
clusters_ENAPRES <- ENAPRES %>%
  count(CONGLOMERADO)
```
...
```{r}
set.seed(10201995)
MAC_ENAPRES <- clusters_ENAPRES %>%
  slice_sample(prop = 0.01)
```

Ahora, juntamos ambas bases: ENAPRES + MAC_ENAPRES, para tener las observaciones de los clusters elegidos
```{r}
MAC_ENAPRES_I <- merge(x = ENAPRES , y = MAC_ENAPRES , by = c("CONGLOMERADO"))
```

### Para la ETAPA II:
```{r}
set.seed(10201995)
MAC_ENAPRES_II <- MAC_ENAPRES_I %>%
  group_by(CONGLOMERADO) %>%
  slice_sample(n = 50)
```

```{r}
table(MAC_ENAPRES_II$ESTRATO)
```
En este caso se indica en el diseño muestral que es una muestra bietapica, para lo cual se identifican los conglomerados (CONGLOMERADO es el identificador de cada conconglomerado). Luego, al interior de cada uno se muestrean las viviendas (VIVIENDA el identificador de cada una de estas). Así, los conglomerados vienen a ser PSU y las viviendas SSU. Entonces, fpc1 tiene el total de conglomerados y fpc2 el total de viviendas.

Procedemos a agregar variables relevantes: fpc1 y fpc2
```{r}
MAC_ENAPRES_II <- MAC_ENAPRES_II %>%
  mutate(fpc1 = 3467 , fpc2 = 380 , .keep = "all")
```

Se declara el diseño muestral:
```{r}
ENAPRES_svy = svydesign(id = ~CONGLOMERADO+VIVIENDA , strata = ~ESTRATO, fpc=~fpc1+fpc2 , weights = ~FACTOR , data = MAC_ENAPRES_II, nest=TRUE)
```
...
(ii) Se elige como variable continua, la varible edad = P208_A
```{r}
...
(i)	Variable dicotómica = REGIONNATU (Selva ==1, Otros ==0): media y población
```{r}
svymean(~REGIONNATU , ENAPRES_svy)
svytotal(~REGIONNATU , ENAPRES_svy)
```

(ii) Variable continua = P208_A: media (representativa)
